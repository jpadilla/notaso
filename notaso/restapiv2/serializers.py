    ...
    def get_extra_info(self, university):
        ...
        extra_info = {
            ...
            ProfessorListSerializer(instance=low_professors,
                                    context=context,
                                    many=True).data,
            'recent_comments': CommentSerializer(instance=comments,
                                                 many=True).data}
        return extra_info
...
class DepartmentRetrieveSerializer(serializers.ModelSerializer):
    ...
    def get_professors(self, department):
        ...
        if university:
            ...
            professors = ProfessorListSerializer(instance=queryset,
                                                 context={'request': request},
                                                 many=True)
...
class ProfessorListSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'url', 'first_name', 'last_name',
                  'slug', 'gender', 'university',
                  'department', 'score', 'grade')
        lookup_field = 'slug'
        extra_kwargs = {
            'url': {'lookup_field': 'slug'}
        }
...
class ProfessorRetrieveSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def get_total_rates(self, professor):
        count = Comment.objects.filter(professor=professor,
                                       responsibility__gt=0).count()
        return count

    def get_user_comments(self, professor):
        queryset = Comment.objects.filter(professor=professor.id).exclude(
    ...
    class Meta:
        ...
        fields = ('id', 'first_name', 'last_name', 'slug',
                  ...
                  'score', 'total_rates', 'grade',
                  'responsibility', 'personality', 'difficulty',
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('created_by', 'body',
                  'is_anonymous', 'responsibility',
                  'personality', 'workload',
